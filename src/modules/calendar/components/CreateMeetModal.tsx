...
export function CreateMeetModal({ isOpen, defaultStart, defaultEnd, onClose, onCreateMeet, inviteNaoufel = false }: Props) {
  ...
  const [startStr, setStartStr] = useState(formatDatetimeLocal(defaultStart ?? in30))
  const [endStr, setEndStr] = useState(formatDatetimeLocal(defaultEnd ?? in60))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const response = await onCreateMeet({
        title: title.trim(),
        start: parseDatetimeLocal(startStr).toISOString(),
      ...
      toast.error(err instanceof Error ? err.message : 'Impossible de créer la réunion')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="relative z-10 w-full max-w-md rounded-[8px] border border-[var(--border-color)] bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-[var(--border-color)] px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--memovia-violet-light)]">
              <Video className="h-4 w-4 text-[var(--memovia-violet)]" />
            </div>
            <span className="text-[15px] font-semibold text-[var(--text-primary)]">
              Nouvelle réunion Google Meet
            </span>
          </div>
          <button
            onClick={handleClose}
        ...
        {result ? (
          ...
          <div className="flex flex-col gap-5 p-5">
            ...
            {result.meetLink ? (
              <div className="rounded-lg border border-[var(--memovia-violet)] bg-[var(--memovia-violet-light)] p-4">
              ...
              <div className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-primary)] px-4 py-3 text-[13px] text-[var(--text-secondary)]">
                Événement créé, mais aucun lien Meet généré par Google pour cet événement.
              </div>
            )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-medium text-[var(--text-secondary)]">
                Titre de la réunion <span className="text-[var(--danger)]">*</span>
              </label>
              <input
                type="text"
                value={title}
            ...
            <div className="flex gap-2 pt-1">
              ...
              >
                ...
                {loading ? 'Création…' : 'Créer la réunion + Meet'}
              </button>
            </div>
          </form>
        )}
